...
class UI(ctk.CTk):
    def __init__(self):
        ...
        self.priority_var = tk.StringVar(value="Medium High")
    ...
    def add_process(self):
        ...
        try:
            ...
            process = self.os.add_process(priority)
    ...
    def create_process_widgets(self, process):
        ...
        # Set a fixed width for the label to make all labels equal in size
        ...
        progress_bar.set(0)
        progress_bar.pack(side=tk.LEFT, padx=(5, 10))  # Increased padding for symmetry

        # Set a fixed width for the dropdown menu (state_menu) and adjust the height if necessary
        ...
        # Store widgets in the process object for future updates
        ...
        for process in self.os.processes:
            if hasattr(process, 'widgets'):
                ...
                if process.widgets["frame"].winfo_exists():
                    ...
                    # If the process is terminated, schedule the removal of its widget after 3 seconds
                    if process.state == ProcessState.TERMINATED:
                        if not hasattr(process, "scheduled_for_removal"):
                            process.scheduled_for_removal = True  # Avoid scheduling multiple removals
    ...
    def start_simulation(self):
        ...
        """
        if self.os.killed:  # If the system was killed, we resume it
            self.os.resume()
        else:  # Otherwise, we start it normally
            self.os.start()
    ...
    def change_process_state(self, pid, new_state):
        ...
        # Cambiar el estado del proceso en el simulador de OS
        self.os.change_process_state(int(pid), new_state)
        
        # Encontrar el proceso en la lista de procesos
        process = next((p for p in self.os.processes if p.pid == pid), None)

        if process:
            # Manejar el estado TERMINATED y eliminar el widget después de 3 segundos
            if process.state == ProcessState.TERMINATED:
                if hasattr(process, 'widgets') and process.widgets and process.widgets["frame"].winfo_exists():
                    self.after(3000, lambda: self.remove_process_widget(process))
            
            # Actualizar el menú desplegable basado en el nuevo estado
            elif process.state == ProcessState.BLOCKED:
